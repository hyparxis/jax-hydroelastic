...
class LinearMeshField:
    def __init__(
        self, mesh: Mesh, values: jax.Array, gradients: Optional[jax.Array] = None
    ):
    ...
    def value_at_barycentric_point(
        self, element_index: int, barycentric_coordinates: jax.Array
    ) -> float:
        indices = self.mesh.elements[element_index]
        return jnp.sum(self.values[indices] * barycentric_coordinates)

    def value_at_cartesian_point(self, element_index: int, point: jax.Array) -> float:
        return (
            self.gradients[element_index].dot(point)
            + self.values_at_origin[element_index]
        )

    def gradient_at_element(self, element_index: int) -> jax.Array:
        return self.gradients[element_index]

    def _compute_value_at_origin(self, element_index: int) -> jax.Array:
        indices = self.mesh.elements[element_index]
        v0_index = indices[0]
        v0 = self.mesh.vertices[v0_index]
        return self.values[v0_index] - self.gradients[element_index].dot(v0)

    def _compute_gradient_vector(self, element_index: int) -> jax.Array:
        indices = self.mesh.elements[element_index]
